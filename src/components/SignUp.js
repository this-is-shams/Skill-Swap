    ...
    const fetchMentorData = async () => {
      try {
        const response = await axios.get(
          "https://localhost:5000/signup/mentor/" // Eta basically backend er index.js and signuphandler dekhlei bujhe jabi. main kaj ta mentor signup handler e hoye ekhane object akare chole ashche.
        );
    ...
    if (uType === "mentor") {
      ...
      async function handleMentorSignup() {
        try {
          const checkResponse = await axios.get(
            `https://localhost:5000/signup/mentor/${user}`
          );
          ...
          if (error.response.status === 500 || error.response.status === 404) {
            fetch("https://localhost:5000/signup/mentor", {
              method: "POST",
      ...
      async function handleMenteeSignup() {
        try {
          console.log("Before")
          const checkResponse = await axios.get(
            `https://localhost:5000/signup/mentee/${user}`
          );
          console.log("CHeck RRR", checkResponse.status)
          alert("User Already Exists! Try a different username");
          ...
          if (error.response.status === 500 || error.response.status === 404) {
            fetch("https://localhost:5000/signup/mentee", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(newService),
            })
