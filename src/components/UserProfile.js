...
export default function UserProfile() {
  ...
  const fetchCPRecords = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/cp/${username}`);
      // console.log(items);
  ...
  const fetchDevRecords = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/dev/${username}`);
      // console.log(its);
      ...
      <div className="h-screen dark:bg-slate-800 dark:text-white pl-60 pt-20">
        <div className="h-full dark:bg-slate-800 flex flex-col items-center">
          <div className="dark:bg-slate-800 flex justify-center mt-8">
            <button
              className={`mr-4 p-4 text-lg font-semibold rounded-lg ${
                toggleState === 1
            ...
            <button
              className={`ml-4 p-4 text-lg font-semibold rounded-lg ${
                toggleState === 4
                  ? "bg-blue-500 text-white"
                  : "bg-gray-300 text-gray-600"
              }`}
          ...
          <div
            className={`${toggleState === 1 ? "block" : "hidden"} text-center`}
          >
          ...
          <div
            className={`${toggleState === 2 ? "block" : "hidden"} text-center`}
          >
