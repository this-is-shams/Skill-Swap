...
export default function UserProfile() {
  ...
  const fetchCPRecords = async () => {
    try {
      const response = await axios.get(`https://back-8c42.onrender.com/cp/${username}`);
      // console.log(items);
  ...
  const fetchDevRecords = async () => {
    try {
      const response = await axios.get(`https://back-8c42.onrender.com/dev/${username}`);
      // console.log(its);
      ...
      <div className="h-screen dark:bg-slate-800 dark:text-white pl-60 pt-20">
        <div className="h-full dark:bg-slate-800 flex flex-col items-center">
          <div className="dark:bg-slate-800 flex justify-center mt-8">
            <button
              className={`mr-4 p-4 text-lg font-semibold rounded-lg ${toggleState === 4
